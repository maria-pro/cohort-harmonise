...
def test_documented_cohorts_ingest_without_any_dictionary_file(cfg):
    ...
    for cid in [c for c, s in cfg.cohorts.items() if s["source"]["kind"] == "documented"]:
...
def test_step3_falls_back_when_item_wording_is_unavailable(cfg):
    ...
    documented = [c for c, s in cfg.cohorts.items() if s["source"]["kind"] == "documented"]
    frames = {}
    for cid in documented:
        spec = cfg.cohorts[cid]
        frames[cid] = normalise.build(ingest.ingest_cohort(spec, cfg.resolve), spec,
                                      cfg.bands, cfg.instruments)
    verdicts, pairs = linkage.assess(frames, cfg, tiers=())
    # A cohort inventoried at domain level has no item wording, so no pair of them can
    # anchor a multi-group model however many there are.
    assert pairs.empty
    if len(documented) > 1:
...
def test_a_construct_name_is_not_an_instrument(cfg):
    ...
    of the SDQ subscale, and of any inventory's domain heading — was an alias, so an
    inventory built from publications looked like it had a named instrument.
    """
    labels = pd.Series(["Emotional symptoms",
                        "emotional symptoms | psychological wellbeing",
                        "Depressive symptoms",
                        "Application Table B1 states SDQ Emotional Symptoms; unconfirmed"])
    probe = pd.DataFrame({"variable": list("wxyz"), "label": labels,
    ...
    assert all(v == "" for v in out["instrument_id"][:3]), \
        "a bare construct name resolved to an instrument"

    # And no publication-based inventory may carry a note into a searchable field.
    for cid, spec in cfg.cohorts.items():
        ...
        assert (df["item_text"].astype(str) == "").all(), \
            f"{cid}: an entry note reached a searchable field"
...
def test_no_cohort_is_credited_with_more_waves_than_it_has(cfg):
    ...
    for cid, spec in cfg.cohorts.items():
        ...
        groups = {w.get("wave_group", w["wave_id"]) for w in primary}
        assert len(groups) == declared, (
            f"{cid}: {len(groups)} primary wave groups but declares {declared} per participant")
