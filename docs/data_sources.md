...
| LSIC (5) | `LSIC Dictionary/2. Data Dictionary - LSIC Release 14.0.xlsx` | AIFS Footprints in Time documentation. **Use requires First Nations governance sign-off**; the tool excludes this cohort unless `--include-governed` is passed |
...
| `reconstructed` | Assembled from published methods and results sections, because no codebook exists. No cohort currently uses this tier; it remains available. |
